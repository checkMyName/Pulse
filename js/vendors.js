(self["webpackChunkgulp4template"] = self["webpackChunkgulp4template"] || []).push([["vendors"],{
...
(function (root, factory) {
  var define = define || {};
  if (typeof define === 'function' && define.amd) define([], factory);else if (true) module.exports = factory();else {}
})(this, function () {
  ...
  FormValidator.prototype = {
    ...
    tests: {
      sameAsPlaceholder: function (field, data) {
        if (field.getAttribute('placeholder')) return data.value != field.getAttribute('placeholder') || this.texts.empty;else return true;
      },
      ...
      email: function (field, data) {
        var i,
            emails = data.value.trim().split(' ');
        if (!field.multiple && emails.length > 1) return this.texts.email;

        for (var i = 0; i < emails.length; i++) if (!this.settings.regex.email.filter.test(emails[i]) || emails[i].match(this.settings.regex.email.illegalChars)) return this.texts.email;

        return true;
      },
      // a "skip" will skip some of the tests (needed for keydown validation)
      text: function (field, data) {
        var that = this; // make sure there are at least X number of words, each at least 2 chars long.
        // for example 'john F kenedy' should be at least 2 words and will pass validation

        if (data.validateWords) {
          var words = data.value.split(' '); // iterate on all the words

          var wordsLength = function (len) {
            for (var w = words.length; w--;) if (words[w].length < len) return that.texts.short;

            return true;
          };

          if (words.length < data.validateWords || !wordsLength(2)) return this.texts.complete;
          return true;
        }
      ...
      number: function (field, data) {
        var a = data.value; // if not not a number
      ...
      date: function (field, data) {
        var day,
            A = data.value.split(/[-./]/g),
            i; // if there is native HTML5 support:

        if (field.valueAsNumber) return true;

        for (i = A.length; i--;) {
          if (isNaN(parseFloat(data.value)) && !isFinite(data.value)) return this.texts.date;
        }

        try {
          day = new Date(A[2], A[1] - 1, A[0]);
          if (day.getMonth() + 1 == A[1] && day.getDate() == A[0]) return true;
      ...
      time: function (field, data) {
        ...
        if (pattern.test(data.value)) return true;else return this.texts.time;
      },
      url: function (field, data) {
        if (!this.settings.regex.url.test(data.value)) return this.texts.url;
        return true;
      },
      hidden: function (field, data) {
        ...
        if (data.pattern) {
          if (data.pattern == 'alphanumeric' && !this.settings.regex.alphanumeric.test(data.value)) return this.texts.invalid;
        }
    ...
    events: function (types, formElm) {
      ...
      if (types instanceof Array) types.forEach(bindEventByType);else if (typeof types == 'string') bindEventByType(types);
    ...
    mark: function (field, text) {
      ...
      var that = this; // check if not already marked as 'bad' and add the 'alert' object.
      ...
      if (this.settings.alerts) {
        if (alert) alert.innerHTML = text;else {
          warning = '<div class="' + this.settings.classes.alert + '">' + text + '</div>';
          item.insertAdjacentHTML('beforeend', warning);
        }
      }

      item.classList.remove(this.settings.classes.bad); // a delay so the "alert" could be transitioned via CSS
    ...
    */
    ...
    testByType: function (field, data) {
      ...
      var type = data.type;
      if (type == 'tel') data.pattern = data.pattern || 'phone';
    ...
    prepareFieldData: function (field) {
      ...
      this.data[id].pattern = field.pattern; // Special treatment

      if (nodeName === "select") this.data[id].type = "select";else if (nodeName === "textarea") this.data[id].type = "text";
    ...
    extend: function (target, varArgs) {
      ...
      for (index = 1; index < arguments.length; index++) {
        ...
        if (nextSource != null) // Skip over if undefined or null
          for (nextKey in nextSource) // Avoid bugs when hasOwnProperty is shadowed
    ...
    */
    checkField: function (field, silent) {
      ...
      if (field.type != 'hidden' && !field.clientWidth) return {
        valid: true,
        error: ""
      };
      field = this.filterFormElements([field])[0]; // if field did not pass filtering or is simply not passed
      ...
      */
      ...
      testResult = this.tests.hasValue.call(this, data.value); // if the field has value, check if that value is same as placeholder
      // if( testResult === true )
      ...
      if (testResult !== true) data.valid = false; // validate by type of field. use 'attr()' is proffered to get the actual value and not what the browsers sees for unsupported types.
      ...
      if (data.valid && data.validateLinked) {
        if (data['validateLinked'].indexOf('#') == 0) linkedTo = document.body.querySelector(data['validateLinked']);else if (form.length) linkedTo = form.querySelector('[name=' + data['validateLinked'] + ']');else linkedTo = document.body.querySelector('[name=' + data['validateLinked'] + ']');
        testResult = this.tests.linked.call(this, field.value, linkedTo.value, data.type);
        data.valid = testResult === true ? true : false;
      }

      if (!silent) this[data.valid ? "unmark" : "mark"](field, testResult); // mark / unmark the field
    ...
    filterFormElements: function (fields) {
      ...
      for (i = fields.length; i--;) {
        ...
        if (isAllowedElement && (isRequiredAttirb || isOptional) && !isDisabled) fieldsToCheck.push(fields[i]);
      }
    ...
    checkAll: function (formElm) {
      ...
      var that = this,
          result = {
        valid: true,
        // overall form validation flag
        fields: [] // array of objects (per form field)

      },
          fieldsToCheck = this.filterFormElements(formElm.elements); // get all the input/textareas/select fields which are required or optional (meaning, they need validation only if they were filled)

      fieldsToCheck.forEach(function (elm, i) {
        var fieldData = that.checkField(elm); // use an AND operation, so if any of the fields returns 'false' then the submitted result will be also FALSE
...
(function (global, factory) {
   true ? module.exports = factory(__webpack_require__(/*! ./dom/data.js */ "./node_modules/bootstrap/js/dist/dom/data.js"), __webpack_require__(/*! ./dom/event-handler.js */ "./node_modules/bootstrap/js/dist/dom/event-handler.js")) : 0;
